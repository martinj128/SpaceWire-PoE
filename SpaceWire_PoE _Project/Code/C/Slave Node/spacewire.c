...
{
    static uint8_t* swPacketBuffer;
    static uint8_t sw_packet_index;
    static uint8_t frameNumber;
    static bool initialFrame = true;
    uint16_t eth_index_start;
    uint16_t eth_index_end;
    
    if(initialFrame) //initlise sw packet buffer and index for first time through
    {
    swPacketBuffer = (uint8_t*)malloc(SW_NCHAR_BUFFER_SIZE);
    sw_packet_index = 0;
    frameNumber = 1;
    initialFrame = false;
    }

    eth_index_start = eth_index;    //get start eth index position
    ...
    while(1)
    {
        ...
        {
            case END_OF_FRAME:
                eth_index_end = eth_index;
                dataRead = eth_index_end - eth_index_start; //end position - start position to find amound of data read inc header and EOP/EOF
                eth_index++;                    //+1 to move address to next headerbyte in loop
                goto exit_loop;
...
char *convertToCharArray(uint8_t *buffer, uint16_t len)
{
    ...
    {
        char_buffer[i] = (char)buffer[i + 14]; //convert uint8_t to char array to print, +14 to skip MAC addr and len bytes
        size++;
    }
